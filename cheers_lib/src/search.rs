                    ...
                    let reduction = {
                            ...
                            && !in_check
                        {
                            fn lmr(depth: i8, move_index: usize, options: &SearchOptions) -> i8 {
                                ((options.lmr_const_reduction as f32 / 100.0)
                                    + (depth as f32).ln() * (move_index as f32).ln()
                                        / (options.lmr_log_divisor as f32 / 100.0))
                                    as i8
                            }
                            r += lmr(depth, move_index, &self.options);

                            if move_score.abs_diff(QUIET_SCORE) < 10_000 {
                                if move_score - QUIET_SCORE
                                    >= self.options.upper_history_threshold as i32
                                {
                                    r -= 1; // reduce historically good quiets less
                                } else if move_score - QUIET_SCORE
                                    <= self.options.lower_history_threshold as i32
                                {
                                    r += 1;
                                }
                            }
